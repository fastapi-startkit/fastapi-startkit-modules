...
class TestHasOneThroughRelationship:
    ...
    async def setup(self):
        ...
        async with (await self.schema.create_table_if_not_exists("incoming_shipments")) as table:
        ...
        await Country().get_builder().bulk_create(
            [
        ...
        await Port().get_builder().bulk_create(
            [
        ...
        await IncomingShipment().get_builder().bulk_create(
            [
    ...
    async def test_has_one_through_eager_load_can_be_empty(self):
        shipments = await (
            ...
            .where_has(
                "from_country", lambda query: query.where("name", "Uruguay")
            )
            .with_("from_country")
            .get()
        )
