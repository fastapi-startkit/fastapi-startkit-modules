...
class BaseTestCaseSelectGrammar:
    ...
    def test_can_compile_sub_select(self):
        to_sql = self.builder.where_in("name", self.builder.new().select("age")).to_sql()
        sql = getattr(self, inspect.currentframe().f_code.co_name.replace("test_", ""))()
        self.assertEqual(to_sql, sql)
    ...
    def test_can_compile_complex_sub_select(self):
        to_sql = self.builder.where_in(
            "name",
            (self.builder.new().select("age").where_in("email", self.builder.new().select("email"))),
        ).to_sql()
        sql = getattr(self, inspect.currentframe().f_code.co_name.replace("test_", ""))()
        self.assertEqual(to_sql, sql)
    ...
    def test_can_compile_exists(self):
        to_sql = (
            self.builder.select("age").where_exists(self.builder.new().select("username").where("age", 12)).to_sql()
        )
        sql = getattr(self, inspect.currentframe().f_code.co_name.replace("test_", ""))()
        self.assertEqual(to_sql, sql)
    ...
    def test_can_compile_not_exists(self):
        to_sql = (
            self.builder.select("age").where_not_exists(self.builder.new().select("username").where("age", 12)).to_sql()
        )
        sql = getattr(self, inspect.currentframe().f_code.co_name.replace("test_", ""))()
        self.assertEqual(to_sql, sql)
    ...
    def test_can_compile_join_clause_with_value(self):
        clause = JoinClause("report_groups as rg").on_value("bgt.active", "=", "1").or_on_value("bgt.acct", "=", "1234")
        to_sql = self.builder.join(clause).to_sql()
    ...
    def test_can_compile_join_clause_with_null(self):
        clause = JoinClause("report_groups as rg").on_null("bgt.acct").or_on_null("bgt.dept").on_value("rg.abc", 10)
        to_sql = self.builder.join(clause).to_sql()
    ...
    def test_can_compile_join_clause_with_not_null(self):
        clause = (
            JoinClause("report_groups as rg").on_not_null("bgt.acct").or_on_not_null("bgt.dept").on_value("rg.abc", 10)
        )
        to_sql = self.builder.join(clause).to_sql()
